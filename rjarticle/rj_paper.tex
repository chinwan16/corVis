...
\CRANpkg{correlationfunnel} assists with the feature selection step by
...
correlation coefficient among the response and the predictors. We
believe that using different association measures is a better approach
than using one-hot encoding for exploring association among all the
variables, as the encoding increases the dimensions very quickly and it
is not easy to assess the overall association.
...
similar to what \citet{tukey1985computer}. They suggested that
...
graph-theoretic scagnostic measures to highlight unusual scatterplot.
