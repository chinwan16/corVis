...
describe the correlation patterns in a dataset.
...
available in R which deals with either calculating correlations,
visualising correlations or both. The packages providing display with
multiple association measures have been marked under the column
\texttt{MultipleMeasures}. Similarly the packages dealing with mixed
variables for correlation displays and the packages displaying
conditional correlation structure have been marked under the columns
\texttt{MixedVariables} and \texttt{ConditionalPlot} respectively. The R
package \CRANpkg{corrplot} \citep{corrplot2021} provides an
...
variable ordering methods which assist in detecting patterns of
relations among the variables. The package \CRANpkg{corrr}
\citep{corrr2020} is useful in calculating as well as visualising
correlations. It calculates a correlation \texttt{dataframe} for a
dataset instead of a matrix and therefore is easier to focus on the
correlations of certain variables. In addition to matrix display, the
package also plots the correlation values in a network display which is
useful when dealing with high-dimensional datasets.

The majority of the packages in Table \ref{tab:corrdisplay-packages}
focus only on numeric variables of the dataset. The package
\CRANpkg{linkspotter} \citep{linkspotter} look at both the numeric as
well as the categorical variables during the correlation analysis. The
package has association measures available for quantitative pairs,
qualitative pairs and mixed pairs. Depending on the variable pairs for
exploring association, a user calculates an association measure and then
visualize these associations using a network plot. The package
\CRANpkg{corVis} also looks at all the possible variable pairs during
the correlation analysis. It allows the user to select association
measure for different variable types and then calculates and visualizes
all of the variable pairs in a single plot. This helps an analyst to
explore the associations among different variables in a single display.

We introduce conditional association displays which are useful in
...
The package \CRANpkg{corrarray} \citep{corrarray} allows a user to
calculate a conditional correlation matrix. The package displays
correlation matrix for one level of the grouping variable in the lower
triangular matrix and displays the correlation matrix for other level in
the upper triangular matrix. For the grouping variable with many levels,
it generates an array of the correlation matrices for each level. The
package \CRANpkg{corVis} also provides a functionality to calculate and
display conditional association in the dataset. A user calculates the
conditional association by specifying a grouping variable and plot these
conditional association measures using the displays provided in the
package.
...
package \CRANpkg{scorrplot} \citep{scorr} produces an interactive
scatterplot for exploring pairwise correlations in a large dataset by
projecting variables as points with respect to some user-selected
variables on a scatterplot, driven by geometric interpretation of
correlation. A user can update variables of interest and can create
tours of the correlation space between different projections of the data
using this tool.

The R package \CRANpkg{correlationfunnel} assists with the feature
selection step by looking at the relationship of predictors to a
response (or a target). It converts numeric predictors into factors,
applies one-hot encoding to all the factors and then calculates and
visualise the Pearson's correlation coefficient among the response and
the predictors. We believe that using different association measures is
a better approach than using one-hot encoding for exploring association
among all the variables, as the encoding increases the dimensions very
quickly and it is not easy to assess the overall association.
...
The tibble output for the functions mentioned in
