...
a matrix display. Effective ordering techniques are also used with these
displays to highlight interesting relationships. The display is useful
to quickly find highly associated variables which are explored further
and are taken into consideration during the modeling step.
...
limited to numeric variables. In order to quickly explore association
among variables in a dataset, an analyst has to use different graphical
displays depending on different variable types and thus calls for a tool
which can summarize associations among all the variables in a single
step.

Many association measures have been proposed to summarize relationship
among two variables. These multiple measures are useful for explaining
complex relationships in a dataset compared to using a single measure.
With effective displaying techniques, the multiple measures of
association provide a comparison tool that assist an analyst in
discovering interesting variable pairs.
...
visualizations which look at multiple association measures, variables of
mixed type, and conditional associations. These displays are implemented
in the R package \texttt{corVis}. The paper is organised in the
following way: first a review of existing packages/methods to calculate
and display association with a quick overiew of some association
measures used in the package, then our approach to calculate the
association measures, then visualizations of associations and
