...
library(ggplot2)
library(mvtnorm)
plot_fun <- function(xy, xlim = c(-4, 4), ylim = c(-4, 4), eps = 1e-15) {
  data <- as.data.frame(xy)
  pearson = round(cor(xy[,1], xy[,2]), 1)
  distance = round(sqrt(energy::dcor2d(xy[,1],xy[,2])), 1)
  MIC = round(minerva::mine(xy[,1],xy[,2])[[1]], 1)
  if (sd(xy[,2]) < eps) title = "" # corr. coeff. is undefined
  ggplot(data=data) + 
    geom_point(aes(x=data[,1],y=data[,2]),size=0.5) + 
...
p1 <- plot_fun(cbind(x,y), xlim = c(-1, 1), ylim = c(-1/3, 1+1/3))
...
p2 <- plot_fun(xy, xlim = c(-lim, lim), ylim = c(-lim, lim))

# diamond pattern
xy = rotation(-pi/8, xy)
p3 <- plot_fun(xy, xlim = c(-sqrt(2), sqrt(2)), ylim = c(-sqrt(2), sqrt(2)))

# quadratic pattern
y = 2*x^2 + runif(n, -1, 1)
p4 <- plot_fun(cbind(x,y), xlim = c(-1, 1), ylim = c(-1, 3))

# x pattern
y = (x^2 + runif(n, 0, 1/2)) * sample(seq(-1, 1, 2), n, replace = TRUE)
p5 <- plot_fun(cbind(x,y), xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5))

# circle pattern
y = cos(x*pi) + rnorm(n, 0, 1/8)
x = sin(x*pi) + rnorm(n, 0, 1/8)
p6 <- plot_fun(cbind(x,y), xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5))
...
p7 <- plot_fun(rbind(xy1, xy2, xy3, xy4), xlim = c(-3-4, 3+4), ylim = c(-3-4, 3+4))
