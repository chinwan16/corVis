...
We use Steel Industry Energy consumption dataset providing information on energy consumption by a steel company in South Korea from the [@Dua:2019] to provide illustrative examples. Table \@ref(tab:steelindustry) provides a brief description of set of variables from this dataset used for analysis.


```{r steelindustry,warning=FALSE,message=FALSE,echo=FALSE }
library(kableExtra)

steelindustry <- read.csv("../data/Steel_industry_data.csv")
...
steelindustry$Load_Type <- factor(steelindustry$Load_Type, levels = c("Light_Load", "Medium_Load","Maximum_Load"))
steelindustry$Load_Type <- ordered(steelindustry$Load_Type)

Variable <- names(steelindustry)
...
Description <- c(
                 ...
                 "Week status (0 for weekend, 1 for weekday)",
                 "Day of the week",
                 "Load type (Light Load, Medium Load or Maximum Load)")

VariableType <- c("numerical","numerical","numerical","numerical","numerical","numerical","numerical","nominal","nominal","ordinal")

df <- data.frame(Variable=Variable,
                 Description=Description,
                 VariableType=VariableType)

kableExtra::kbl(df, booktabs = T, caption = "Variable description of the Steel Industry Energy consumption dataset along with the types of variables")
#print(table_1)
#df %>%
  #kbl() %>%
  #kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
...
```{r assoc-heatmap,fig.width=5, fig.height=4, fig.align='center', fig.cap="Association matrix display for energy consumption data showing Pearson's correlation for numeric variable pairs, canonical correlation for mixed variable pairs and categorical variable pairs.",echo=TRUE}
...
```{r numeric-pairs, fig.width=5, fig.height=4, fig.align='center', fig.cap = "Scatterplot for variables Usage_kWh and Co2.tCO2., and, LdCRP and LdCPF showing a high positive and negative Pearson's correlation respectively."}

p1 <- ggplot2::ggplot(data = df) + ggplot2::geom_point(ggplot2::aes(y=Usage_kWh,x=CO2.tCO2.),size = 0.2)

p2 <- ggplot2::ggplot(data = df) + ggplot2::geom_point(ggplot2::aes(y=LdCRP,x=LdCPF),size = 0.2)
...
```{r mixed-pairs, fig.width=5, fig.height=4, fig.align='center', fig.cap = "Boxplot for variables Load_Type and Usage_kWh, and, Load_Type and NSM showing a high canonical correlation. "}

p3 <- ggplot2::ggplot(data = df) + ggplot2::geom_boxplot(ggplot2::aes(y=Usage_kWh,x=Load_Type))

p4 <- ggplot2::ggplot(data = df) + ggplot2::geom_boxplot(ggplot2::aes(y=NSM,x=Load_Type))
...
compare_assoc_steel$measure_type <- factor(compare_assoc_steel$measure_type,
                                           levels = c("cancor","mic","dcor","pearson","spearman"))
pairwise_1d_compare(compare_assoc_steel)

```


```{r comp-var-pairs, fig.width=4, fig.height=4, fig.align='center', fig.cap = "Scatterplot for numeric variable pairs with high difference among the magnitude of association measures"}
...
```{r high-diff, fig.width=5, fig.height=4, fig.align='center', fig.cap = "Scatterplot for variable pair LgCPR and Usage_kWh splitted by a conditioning variable htn."}
