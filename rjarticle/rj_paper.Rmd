...
The new package \CRANpkg{corVis} overcomes these shortcomings of existing correlation matrix displays by providing association measures and visualisations for datasets with mixed variable types and non-linear associations. We also calculate and visualise measures of association at different levels of a categorical variable to explore patterns at the levels of the conditioning variable. In addition to matrix layout, we also provide linear layouts for each of these displays which are useful in showing filtered variable pairs with strong associations.

In this work, we use association measures suitable for both numerical and categorical variables. We focus on four types of variable pairs which are numeric, factor, ordinal and mixed. There are no measures available for numeric and ordinal variable pairs in the literature. One could use Kendall or Spearman rank correlation by numerically coding the levels of the ordinal variable. We refrain from using this strategy and treat these pairs as mixed in the package. 

For numeric variable pairs, we go beyond Pearson, Spearman and Kendall correlation, and use methods such as distance correlation [@szekely2007measuring], maximal information coefficient (MIC) [@reshef2011detecting]], ace [@wang2005identifying] from alternating conditional expectations algorithm and more, which are capable of capturing non-linear patterns. We calculate and compare multiple measures for each pair of variables so that patterns other than linear association can be uncovered.

In \CRANpkg{corVis}, we also calculate bivariate association measures at different levels of a categorical variable for every variable pair in a dataset. On comparison of these measures, we can discover variable pairs showing different patterns at different levels of the grouping or conditioning variable. 
...
While other packages exist which offer association measures and displays for datasets with different variable types and non-linear associations, we provide a new tibble data structure for the output of association measures of different variable types, multiple measures of association and conditional association measures. We also offer new variants of matrix and linear displays with careful seriation designed to improve interpretability.
