...
# wave pattern
yw = 4 * (x^2 - 1/2)^2 + runif(200, -1, 1)/3


# quadratic pattern
yq = 2*x^2 + runif(200, -1, 1)




# x pattern
ycross = (x^2 + runif(n, 0, 1/2)) * sample(seq(-1, 1, 2), n, replace = TRUE)

# circle pattern
ycir = cos(x*pi) + rnorm(n, 0, 1/8)
xcir = sin(x*pi) + rnorm(n, 0, 1/8)

data <- tibble(dat = c(list(data.frame(x=xlp,y=ylp)),
                       list(data.frame(x=xln,y=yln)),
                       list(data.frame(x=x_r4,y=y_r4)),
                       list(data.frame(x=x,y=yc)),
                       list(data.frame(x=x,y=yw)),
                       list(data.frame(x=x,y=yq)),
                       list(data.frame(x=x,y=ycross)),
                       list(data.frame(x=xcir,y=ycir))
                       ),
               
               pattern = c("linear positive", "linear negative", "x^(1/4)", "cubic", "wave",
                           "quadratic", "X", "circle"),
               
               cor = c(0.9, -0.7, 0.8, 0.7, 0, 0, 0, 0),
               dcor = c(0.9, 0.7, 0.8, 0.7, 0.3, 0.3, 0.2, 0.2),
               MIC = c(0.7, 0.4, 0.7, 0.4, 0.7, 0.4, 0.4, 0.4)
               )

#ggplot(data=data$dat[[1]]) + geom_point(aes(x=x,y=y))
...
for (i in 1:nrow(data)) {
  ...
  dat <- data$dat[[i]]
  p1 <- ggplot(data = dat) + geom_point(aes(x=x, y=y),size=0.5) + coord_fixed() + theme_void()
  #plots <- append(plots,list(p1))
  
  p2 <- ggplot(data=dat) + 
    theme_void() + 
    geom_text(aes(0,0, label= paste0( names(data)[3], "=", round(cor(dat[,1], dat[,2]), 1) ))) +
    coord_fixed()
  #plots <- append(plots,list(p2))
  
  p3 <- ggplot(data=dat) + 
    theme_void() + 
    geom_text(aes(0,0, label= paste0( names(data)[4], "=", round(sqrt(energy::dcor2d(dat[,1],dat[,2])), 1)))) +
    coord_fixed()
  #plots <- append(plots,list(p3))
  
  p4 <-  ggplot(data=dat) + 
    theme_void() +
    geom_text(aes(0,0, label= paste0( names(data)[5],"=", round(minerva::mine(dat[,1],dat[,2])[[1]], 1)))) +
    coord_fixed()
  #plots <- append(plots,list(p4))
  
  plots[[i]] <- grid.arrange(p1,p2,p3,p4, ncol =4)
  
}

do.call("grid.arrange", c(plots, nrow = 8))
...
To motivate our work on using multiple association measures, consider the scatterplots in Figure \@ref(fig:motivation1) which show different simulated linear and non-linear patterns . Each row of the plot shows the simulated pattern, value for Pearson's correlation, distance correlation and maximal information coefficient respectively. It is evident from Figure \@ref(fig:motivation1) that Pearson's correlation and distance correlation produce similar results for patterns with linear relationships. For majority of the non-linear patterns in Figure \@ref(fig:motivation1), MIC seems to be the measure which looks suitable in summarizing the relationships. There are a few non-linear patterns where distance correlation may be the most suitable association measure. This suggests that association measures such as distance correlation, MIC and other measures should be used along with Pearson's correlation for exploring relationships among variables.
...
Function <- c("calc_assoc","association_heatmap","pairwise_2d_plot","pairwise_1d_plot")

Usage <- c("Calculation","Visualization","Visualization","Visualization")
...
Description <- c("Calculates association measures",
           ...
           "Conventional correlation matrix plot",
           
           "Conditional and multiple measures matrix plot",
           
           "Conditional and multiple measures linear plot")
