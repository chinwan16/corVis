...
assoc <- calc_assoc(df)
plot_assoc_matrix(assoc, var_order = names(df))
...
```{r multi_display, echo=TRUE, fig.align='default',fig.cap="Figure 4: Seriated multiple association measures display for acs12 data. Variable pairs at top left or along diagonal of the display have a high value for any of the multiple measures."}
...
```{r cond_display, echo=TRUE, fig.align='default', fig.cap="Figure 5: Seriated conditional association measures display for acs12 data. Variable pairs at top left or along diagonal of the display have a high difference in the measures value calculated at different levels of the conditioning variable."}
cond_assoc <- calc_assoc(df,by="edu",include.overall = F)
