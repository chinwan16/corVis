...
```{r fig.width=4, fig.height=5, fig.align='center',fig.cap="Figure 2: Comparing multiple scagnostic measures using a linear layout"}
...
scag_measures <- rbind(outlying, skewed, clumpy, sparse, striated, convex, skinny, stringy,
                       monotonic)
pairwise_1d_compare(scag_measures)
```

### Conditional Scagnostics

The package includes a function `calc_assoc_by` which calculates the pairwise association at different levels of a categorical conditioning variable. This helps in finding out interesting variable triples which can be explored further prior to modeling. Figure 3 shows a conditional plot for the *penguins* data. Each cell corresponding to a variable pair shows three bars which correspond to the monotonic scagnostic measure calculated at the levels of conditioning variable *species*. The dashed line represents the overall value of monotonic measure.

Figure 3 clearly shows that there is a strong relationship between bill depth and bill length for Chinstrap and Gentoo penguins which is not evident when we look at the overall relationship between bill depth and bill length (Figure 2).  


```{r fig.width=4, fig.height=4, fig.align='center', fig.cap="Figure 3: Conditional scagnostics plot"}

df_num <- dplyr::select(df, where(is.numeric),"species")

updated_assoc <- update_assoc(num_pair = "tbl_scag",num_pair_argList = "Monotonic")

cond_assoc <- calc_assoc_by(df_num, types = updated_assoc, by="species")
