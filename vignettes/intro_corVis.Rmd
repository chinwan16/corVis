...
dplyr::glimpse(association_measure(data = df[numeric_vars],method = "pearson"))
...
```{r}
distance <- association_measure(data = df,group_by = "Classification",method = "distance")
```
...
```{r}
pearson <- association_measure(data = df,group_by = "Classification",method = "pearson")
...
Heatmap display has been seriated by the maximum difference between the measures. Therefore, variable pairs with highest difference will be displayed on the top and the pairs with lowest difference at the bottom. This will help an analyst to quickly spot the variable pairs with highest difference, and can investigate the reasons behind this difference.
```{r}
all_measures <- association_measures(df[numeric_vars])
...
all_measures <- association_measures(df,group_by = "Classification")
heatmap_measures_Viz(all_measures,group_by="Classification")
```
...
```{r}
pearson_measure <- association_measure(df, group_by = "Classification", method = "pearson")

funnel_plot(pearson_measure)
```


### Funnel plot for a designated response and predictors

```{r}
funnel_plot(pearson_measure,response = "Resistin")
```
