...
dim(NHANES)
NHANES <- NHANES %>% dplyr::select(-ID)
nhanes_mod <- NHANES %>% select(Age,SleepHrsNight,BPSys1,Depressed,Diabetes,TotChol,PhysActive,HealthGen)
...
#update the measures
updated_assoc <- update_assoc(num_pair = "tbl_cor",num_pair_method = "spearman",mixed_pair = "tbl_cancor",other_pair = "tbl_nmi")
glimpse(calc_assoc(nhanes_mod,types = updated_assoc))
```
...
a <- rbind(pearson,spearman)
pairwise_summary_plot(a, group_var = "measure_type")
```


We can also use linear layouts for comparing multiple measures. 

```{r fig.width=8, fig.height=8, fig.align='center',fig.cap="Figure 3: Comparing multiple association measures using a linear layout"}

updated_assoc <- update_assoc(num_pair = "tbl_cor",mixed_pair = "tbl_cancor",other_pair = "tbl_nmi")
a <- calc_assoc(nhanes_mod,updated_assoc)
updated_assoc <- update_assoc(num_pair = "tbl_cor",num_pair_method = "spearman",mixed_pair = "tbl_nmi",other_pair = "tbl_nmi")
b <- calc_assoc(nhanes_mod,updated_assoc)
updated_assoc <- update_assoc(num_pair = "tbl_cor",num_pair_method = "kendall",mixed_pair = "tbl_nmi",other_pair = "tbl_nmi")
c <- calc_assoc(nhanes_mod,updated_assoc)
