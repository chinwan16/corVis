...
#' @param var_order a character string for the ordering of the variables. Either "default" (default) or "max_diff"
#' @param limits a numeric vector of length $2$ specifying the limits of the scale. Default is c(-1,1)
#' @param highlight a character vector or a dataframe
#' @export
#' @importFrom magrittr %>%
...
pairwise_2d_plot_sp_high <- function(lassoc, uassoc=NULL, group_var = "by",fill="default",
  ...
  if(length(highlight)==1){
    ...
    highlight_df <- assoc[1:nrow(highlight),]
    highlight_df$x <- highlight$x
    highlight_df$y <- highlight$y
    highlight_df$score <- highlight$score
    highlight_df$measure <- NA
    highlight_df$intercept <- NA
    assoc$intercept <- 0
    assoc$score <- NA
    assoc <- rbind(assoc,highlight_df)
  ...
  if(length(highlight)==1){
    p <- p + ggplot2::geom_rect(mapping=ggplot2::aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf),
                                data=simpson[,1:2],fill = 'red', alpha = 0.1)
  }else{
    p <- p +
      ggplot2::geom_rect(ggplot2::aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf,size=.data$score),fill=NA,color="skyblue") +
